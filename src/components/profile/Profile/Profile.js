...
const LazyImage = (imageProps) => {
  ...
  return shouldLoad ? (
    <img {...imageProps} alt="profile" className="rounded-full" />
  ) : (
...
const Profile = () => {
  ...
  }, [user?.access_token, user?.isAuthenticated]);
        ...
        <div className="bg-white w-16 h-[70px] rounded-md flex flex-col gap-1 text-[#33475b] justify-center items-center shadow-testShadow overflow-x-hidden group cursor-pointer">
          <div className="flex justify-center items-center">
            {user?.isAuthenticated ? (
              <LazyImage src={"https://i.pravatar.cc/300"} />
            ) : (
              <>
          ...
          <p className="text-xs font-semibold font-titleFont">
            {user?.isAuthenticated ? profile?.name : "Profile"}
          </p>
        </div>
      </Link>
      <Link to="/cart">
